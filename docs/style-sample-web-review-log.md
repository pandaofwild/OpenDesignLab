...
| 45 | 자연 / 수공예 | botanical | organic-brand | verified | GLASSHOUSE 허바리움 대지(mount sheet) — 무드보드(압화/허바리움)는 이미 재작업돼 있었으나 샘플·이미지가 구 '식물샵 인테리어' 클리셰로 남아 있던 것을 완성. **압화 몬스테라 한 장이 지면을 지배**하고 대지 위에 허바리움 라벨(accession·과·압착연도·시트번호)이 마운트됨. 오른쪽은 헤어라인 괘선만 쓴 종(species) 기록 — 라틴 이명 이탤릭 세리프 헤드라인, 재배 조건 4행, 가격+CTA, "also under glass" 3종. 박스·공용 하단 스트립·마이크로 아이콘 전부 제거 |
...
| 51 | 귀여움 / 캐주얼 | dopamine-design | kawaii-app | verified | SATURATE 'SPECTRUM SHOP' 단일안료 과슈 상점으로 재설계: 색 스펙트럼 자체가 필터 축(color filter spine, 선택 상태·결과 카운트 연동)이고 화면의 모든 색면이 실제 SKU(안료 코드 PR122·불투명도·내광성 LF·가격·add). 스펙트럼 spine 한 곳에만 볼드를 쓰고 카탈로그는 화이트·헤어라인으로 규율. 우측은 colorway cart(선택 안료·mixes to 램프·소계·체크아웃) + ready set shelf(3세트·색 램프·가격). 구 'PULSE' 스트릭 대시보드(빈 그라디언트 슬래브 2개·장식용 무지개 바·Bronze/Silver/Gold 티어) 폐기 |
...
- 이미지 교체: `scripts/gen-style-image.mjs`의 `botanical` 프롬프트를 "식물샵 인테리어"에서 **압화 몬스테라 잎 매크로 사진**(크림 허바리움지 위, 글라신 시트, 잎맥·폭싱 자국)으로 전면 교체 후 재생성. 무드보드 방향과 샘플 이미지가 처음으로 일치.
- **1차 시도는 실패했고 소유자가 반려했다("이게 맞아?"). 기록해 둘 가치가 있는 오독이다:**
  1. "이미지는 최소화"를 **장수 줄이기로만 읽고 한 장 남은 사진을 56px 썸네일로 축소**했다. 뭐가 찍혔는지 안 보이면 없는 것과 같다. 올바른 해석은 **장수는 줄이되 남은 한 장은 크게** — 이 스타일에서 압화 표본 사진이 곧 보태니컬 신호이기 때문이다.
  2. 그 결과 식물성을 **어휘(라틴 학명)가 100% 짊어지고 형태는 아무것도 하지 않는** 상태가 됐다. 단어를 지우면 연녹색 관리자 테이블이다 → [[style-visibility-over-vehicle]] 위반. 이 저장소의 "한눈에 알아볼 것" 기준은 *형태*가 스타일을 말해야 한다는 뜻이다.
  3. 같은 크기 상자 3개를 쌓은 구조는 AGENTS.md가 명시적으로 금지한 "고른 윤곽 박스"이고, 각 행 가운데가 텅 비어 밀도도 없었다. 하단 스트립은 카테고리 공용이라 rustic·handmade와 또 같아졌다.
  4. 종별 잎 구조 SVG 아이콘 4종을 그렸지만 **16px에서는 구분이 안 되어** 그냥 점으로 읽혔다 — "장식적 기하 대체물" 실패 사례. 전부 제거했다.
- 컴포넌트 `BotanicalGlasshouse`(인라인, `DesignStyleSampleRenderer.tsx`) 2차 = 채택안: **허바리움 대지(mount sheet)** 구조. 보태니컬의 진짜 레이아웃 문법은 "큰 압화 표본이 대지를 지배하고 라벨이 구석에 마운트된다"이고, 이건 이미지를 한 장만 쓰면서 오히려 크게 쓰는 것이 정답이 되는 형태다. 좌측 = 압화 잎 전체 높이 + 대지 위 라벨(Glasshouse herbarium / 학명 / Araceae·Pl. 014 / Pressed vi.1994·Sheet a/3), 우측 = 헤어라인 괘선만으로 구성한 종 기록(kicker → 이탤릭 세리프 학명 2줄 → 통칭 → 재배 조건 4행 → $68+ADD TO ORDER → also under glass 3종). 박스 0개, 공용 하단 스트립 제거.
- 튜닝 3건:
  1. 모바일 390px에서 `SampleNav`의 긴 `sub`("Verified species, not category")가 CSS Grid 기본 `min-width:auto` 때문에 프레임을 밀어냄(scrollWidth 410 vs clientWidth 297) → `min-w-0` 전달 + `sub`를 "Est. 1994"로 축약 + 링크 3→2개.
  2. compact(248px)에서 하단이 잘림. 원인은 `grid-rows-[auto_1fr]`의 **`1fr`이 기본적으로 `minmax(auto,1fr)`이라 우측 컬럼의 콘텐츠 높이가 행을 밀어낸 것** → `grid-rows-[auto_minmax(0,1fr)]`로 교정. 이 함정은 이 저장소에서 반복 등장하므로 기억해 둘 것.
  3. 390px에서 2단 분할(297px 폭)이 학명 헤드라인을 감당 못함 → 모바일은 **대지 위 / 기록 아래로 스택**(`grid-rows-[minmax(0,1fr)_auto] sm:grid-cols-[1.2fr_0.8fr] sm:grid-rows-1`), 헤드라인·라벨도 `sm:` 브레이크포인트로 이원화. 샘플 프레임은 297px지만 뷰포트는 390px이므로 `sm:`(640px)가 모바일/데스크톱 분기로 정확히 동작한다.
- 검증: 1440 full(700×700)·390 mobile(297×297)·compact 카드(503×248) 모두 프레임 overflow 0, 콘텐츠 박스 밖으로 나간 요소 0, console error 0(다른 스타일들의 React key 경고 3건은 이번 변경과 무관). `check:data`(76)·`check:style-distinction`(76)·`check:style-refs`(76)·`lint`·`tsc --noEmit`·`build`(544 pages) 통과.
- 남은 의심점: 없음.
## 37. kawaii · playful-design 제거 (2026-08-03, 소유자 지시: "Kawaii와 플레이풀 디자인을 없애자")

- 귀여움/캐주얼 카테고리 9개 → **7개**(kitsch, dopamine-design, pop-art, comic-book-style, toy-design, pastel-style, bubble-design). 전체 스타일 76 → **74**, `check-data` 하한도 76 → 74.
- 제거 범위(선례인 kinfolk·wabi-sabi 커밋과 동일한 경로): designStyles.ts의 팔레트·시드행·research 브리프·styleTokenOverrides·무드보드 레코드, 렌더러의 샘플 컴포넌트와 라우팅과 이미지 맵, `check-cute-casual`·`check-style-distinction`·`check-data`, `style-references.json`, `gen-style-image.mjs` 프롬프트, 생성 자산 3개, 구분표·리뷰로그 행, 추천 스킬.
- **함정 1 — `paletteBank` 길이 보존:** `palettes.kawaii`가 `paletteBank`에서도 참조되고 있었고, 이 배열은 `paletteBank[hash(slug) % paletteBank.length]`로 인덱싱된다. 항목을 그냥 지우면 **자체 팔레트가 없는 모든 스타일의 색이 재배정된다.** 그래서 해당 항목을 같은 값의 리터럴로 인라인해 길이를 11로 유지하고, 이유를 코드 주석으로 남겼다. HEAD와 현재 모두 11개로 확인.
- **함정 2 — `kawaii-app`은 스타일이 아니라 sampleType:** kitsch·dopamine-design·comic-book-style·toy-design·pastel-style·bubble-design·claymorphism이 이 sampleType을 쓰고 있어 `KawaiiApp` 폴백 컴포넌트와 타입 유니온 멤버는 **유지**했다. 이름만 보고 지웠으면 7개 스타일이 깨졌다.
- 죽은 상호참조 정리(kinfolk 선례 따름): 형제 스타일의 `avoidTraits` 4건, 로코코 한국어 상세 문구 1건, 렌더러의 이웃-차별화 주석 6건, 구분표의 "겹치기 쉬운 스타일" 열 5건, `style-references.json`의 "Distinguish it from ..." 문구 4건, `check-cute-casual`의 금지 마커 3건에서 사라진 두 슬러그를 제거하거나 살아있는 이웃으로 교체했다. 무드보드 생성 프롬프트 속 "not kawaii softness" 류는 **그대로 두었다** — 실제 그 이미지를 생성한 프롬프트의 기록이라 고치면 이력이 왜곡된다.
- 검증: `check:data`(74)·`check:cute-casual`(7)·`check:style-distinction`(74)·`check:style-refs`(74)·`lint`·`tsc --noEmit`·`build`(544 → **538 pages**, 2 스타일 × ko/en/무로케일 = 6감소) 통과. 브라우저에서 `/styles` 목록에 두 스타일 부재·귀여움 카테고리 7개 확인, `/styles/kawaii`·`/styles/playful-design` 404 확인, `kawaii-app` sampleType 이웃(kitsch) 정상 렌더·console error 0 확인.
## 38. dopamine-design 재설계 — 스트릭 대시보드 → SPECTRUM SHOP (2026-08-04, 소유자 지시: "Dopamine Design도 리디자인하자")

- **구 샘플('PULSE')의 문제:** (1) 큰 카드 2개가 제목과 진행바만 얹힌 **빈 그라디언트 슬래브**로 화면의 40%를 차지 — AGENTS.md가 금지한 "평면 원색 블록이 콘텐츠를 대신하는" 실패. (2) 원형 링·카드·스펙트럼 바·미터가 **전부 그라디언트**라 시그니처가 없음(볼드는 한 곳에만). (3) 하단 "dopamine spectrum" 무지개 바는 슬라이더인 척하는 순수 장식, Bronze/Silver/Gold 티어는 전형적 게임화 클립아트. (4) 무엇보다 **스트릭+XP+티어는 "도파민=듀오링고"라는 가장 정석적인 매핑**이었다.
- **근거 재확인:** dopamine의 referenceSites는 **Happy Socks · BAGGU · Duolingo** — 셋 중 둘이 색 중심 커머스인데 샘플은 Duolingo 하나만 따라갔다. natural 때와 동일한 "referenceSites 미확인" 실수의 재발이다.
- 소유자에게 vehicle 3안(색 커머스 / 채도 높은 페인트 하우스 / 실행만 재건)을 제시해 **색 커머스**를 선택받았다. 지난 botanical 건에서 컨셉을 혼자 정했다가 반려된 경험을 반영해, 컨셉 분기만 먼저 확인하고 실행은 내가 가져갔다.
- 컴포넌트 `DopamineSpectrumShop`(구 `DopamineRewardLoop` 삭제): **SATURATE — 단일안료 과슈 상점**. 색 스펙트럼 12칩이 실제 필터 축(`color filter spine`, 선택 칩은 높이+링, 옆에 `38 pigments · magenta` 결과 카운트)이고, 그 아래 안료 카탈로그 6종(안료 코드 PR122/PO73/PY74/PG7/PB15/PV23 · 불투명도 · 내광성 LF · 가격 · add). 우측 레일은 `colorway cart`(선택 안료 3점·mixes to 램프·소계 $25·체크아웃) + `ready set shelf`(3세트·색 램프·가격). **핵심 원리: 채도가 정당한 이유는 모든 색면이 장식이 아니라 실제 SKU이기 때문**이다. 볼드는 스펙트럼 spine 한 곳에만 쓰고 카탈로그는 화이트·헤어라인으로 규율했다.
- 데이터 동기화: 팔레트를 샘플과 일치시킴(base `#FDFCFF` / primary `#FF2E93` 마젠타 CTA / accent 버밀리언·그린·앰버 — 구 팔레트는 노랑 바탕+초록 primary로 샘플과 어긋나 있었다), representativeTraits 5개를 전부 실재 모듈명으로 교체, avoidTraits에 폐기한 경로("Gamified streak dashboard")를 안티레퍼런스로 추가, tokenIntent 재작성. 구분표 행도 "Colour sold as the product / SPECTRUM SHOP"으로 갱신. 무드보드는 **교체하지 않았다** — 채도 높은 컬러블록 프루프와 하단 스펙트럼 칩 스트립이라 새 방향과 오히려 더 잘 맞는다.
- 체크 스크립트: 함수명·필수 마커(`SPECTRUM SHOP`/`color filter spine`/`saturated swatch grid`)·경험 마커(`SINGLE PIGMENT HOUSE`/`colorway cart`/`pigment record`/`family result count`) 교체, 폐기한 구 마커(`STREAK ENERGY ENGINE`/`habit orbit`/`reward ladder`)를 forbiddenPrototypeCopy에 추가해 회귀 방지. 형제 kitsch·pastel-style의 금지 목록이 가리키던 구 마커 `COLOR REWARD LOOP`도 `SPECTRUM SHOP`으로 교체.
- **부수 발견 — 전 스타일에 걸린 compact 잘림 버그:** compact 카드에서 가격/ADD 행이 잘려 원인을 파니, `SampleFrame`의 base 클래스에 있던 `min-h-[250px]`가 compact 분기의 `min-h-[210px]`와 같은 명시도로 캐스케이드에 올라가 **소스 순서로 250px가 이기고 있었다**(`cn`은 tailwind-merge가 아니라 단순 join이라 뒤 클래스가 이기지 않는다). 프리뷰 박스는 222px여서 **74개 스타일 전부 하단 약 28px이 잘리고 있었다.** base의 min-height를 제거해(양쪽 분기가 각자 값을 이미 선언) 해결. 수정 후 74개 프레임 모두 220px ≤ 222px, 잘림 0건 — dopamine뿐 아니라 kitsch의 하단 레일, pop-art의 halftone caption rail도 이제 온전히 보인다.
- 검증: 1440 full(698×698, 프레임 내부 잘림 0)·390 mobile(295×295, 스와치 2열로 6종 전부 노출)·compact 카드(501×220) 모두 overflow 0, console error 0. `check:data`(74)·`check:cute-casual`(7)·`check:style-distinction`(74)·`check:style-refs`(74)·`lint`·`tsc --noEmit`·`build`(538 pages) 통과.
- 남은 의심점: 없음.
